...
/**
 * Cliente HTTP para a API OpenAI.
 *
 * Implementa ClienteLLMInterface e realiza chamadas ao endpoint
 * /chat/completions para gerar DocBlocks PHPDoc via modelos GPT.
 */
final class ClienteOpenAI implements ClienteLLMInterface
{
    /**
     * Envia um prompt ao modelo OpenAI e retorna o DocBlock gerado.
     *
     * @param string $sBase    URL base da API.
     * @param string $sApiKey  Chave de autenticação Bearer.
     * @param string $sModelo  Nome do modelo (ex: gpt-4o-mini).
    ...
    public function gerar(string $sBase, string $sApiKey, string $sModelo, string $sPrompt, string $sSystem = ''): ?string
    {
        ...
        $aPayload = [
            'model' => $sModelo,
            'messages' => [
                ['role' => 'system', 'content' => $sSystemMsg],
